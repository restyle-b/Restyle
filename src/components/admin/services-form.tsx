...
import { updateServices } from "@/server/actions/admin/services";
import { buttonVariants } from "@/components/ui/button";
import { cn } from "@/lib/utils";

const formSchema = z.object({ rows: serviceSchema.array() });
type FormValues = { rows: ServiceInput[] };
...
function emptyRow(order: number): ServiceInput {
  return {
    slug: "",
    order,
    nameHe: "",
...
export function ServicesForm({ initialValues }: { initialValues: ServiceInput[] }) {
  ...
  async function onSubmit(values: FormValues) {
    ...
    const result = await updateServices(values.rows);
    setServerMessage(
        ...
        {fields.map((field, index) => (
          <div key={field.id} className="rounded-lg border border-line-dark p-4">
            <div className="mb-3 flex items-center justify-between">
              <span className="text-sm text-neutral-400">שירות #{index + 1}</span>
              <button
                type="button"
