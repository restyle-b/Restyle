...
| 2026-06-18 | **לוגו/Wordmark**: לפי תמונה מהמשתמש — סמליל "ReStyle" בגופן serif אלגנטי. הוטמע **Playfair Display** (`--font-logo`), נוצרה קומפוננטת `Wordmark` ("ReStyle") בשימוש ב-header (ימין, RTL) וב-footer; `siteConfig.name` נשאר "Restyle" לטקסט/SEO. typecheck/lint/build ✅, אומת ויזואלית (Playwright). | אם הגופן לא מדויק — לשקול Cormorant/DM Serif Display/Marcellus. |
