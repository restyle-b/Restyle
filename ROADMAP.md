...
| 2026-06-19 | **נוסף סקיל חמישי — `tranzila-payments`** (בקשת המשתמש: "תוסיף סקיל מיוחד" לחיבור סליקת אשראי, בציון שזה קריטי 10/10 — באג כאן "האתר יקום ויפול עליו"). נוצר `.claude/skills/tranzila-payments/SKILL.md`: מנחה להעדיף hosted-fields/iframe של טרנזילה (לא להעביר פרטי כרטיס בשרת שלנו → scope PCI מצומצם), זרימת עסקה עם אימות סכום בשרת מה-DB, אימות callback/webhook (חתימה או קריאת status נוספת — לא לסמוך על redirect בצד-לקוח), idempotency, אי-אחסון PAN/CVV, הפרדת סביבת sandbox/production, וצ'קליסט אבטחה ייעודי שמופעל **לצד** סקיל `security`, לא במקומו. עודכן `CLAUDE.md` §"הסקילים" (4→5 סקילים). אין עדיין קוד תשלומים בפועל — זה תכנון-מתודולוגי בלבד, ממתין לפתיחת Phase 4/5 (הרחבה אופציונלית) ולפרטי API מהמשתמש. | אם/כשהלקוח יבחר בהרחבה (חנות+תשלומים): `planning` ייעודי לפי הסקיל החדש לפני קוד; המשתמש הציע עזרה ישירה מניסיון קודם בחיבור טרנזילה עם Claude. שאר הפריטים הפתוחים ללא שינוי. |
